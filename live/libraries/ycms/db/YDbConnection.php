...
/**
 * YDbConnection(Database Connection Manager) class is a manager of database connections.
 ...
 * */
class YDbConnection extends CDbConnection
{
        ...
        {
                if ( $this -> enableSlave && ! $this -> getCurrentTransaction () && self::isReadOperation ( $sql ) && (($slave = $this -> getSlave ()) !== null))
                {
                         $slave-> createCommand ( $sql );
                }
                else
                {
