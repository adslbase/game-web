...
class GxcHelpers {
        ...
        public static function getPayways($render_view = false) {
                $cache_id = $render_view ? 'pay-way' : 'pay-way-false';
                $types = Yii::app()->cache->get($cache_id);

                if ($types === false) {
                        $types = array();
                        $folders = get_subfolders_name(Yii::getPathOfAlias('common.payway'));
                        foreach ($folders as $folder) {
                                $temp = parse_ini_file(Yii::getPathOfAlias('common.payway.' . $folder . '') . DIRECTORY_SEPARATOR . 'info.ini');
                                if ($render_view)
                                        $types[$temp['class']] = $temp['name'];
                                else
                                        $types[$temp['class']] = $temp;
                        }
                        Yii::app()->cache->set($cache_id, $types, 7200);
                }
                return $types;
        ...
        public static function renderLinkPreviewResource($data) {
                switch ($data->resource_type) {
                        case 'image':
                                return '<a href="' . $data->getFullPath() . '" rel="prettyPhoto" title="' . $data->resource_name . '">' . t('查看') . '</a>';
                                break;
                        case 'video':
                                return '<a href="' . app()->controller->backend_asset . '/js/jwplayer/player.swf?width=470&amp;height=320&flashvars=file=' . $data->getFullPath() . '" title="' . $data->resource_name . '" rel="prettyPhoto">' . t('查看') . '</a>';
                                break;
                        default:
                                return '<a href="' . $data->getFullPath() . '">' . t('查看') . '</a>';
                                break;
                }
        }
