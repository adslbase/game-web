<?php
        ...
        class Resource extends CActiveRecord
        {
                ...
                public function attributeLabels()
                {
                        return array(
                                    'resource_id' => t('Id'),
                                    'resource_name' => t('附件名称'),
                                    'resource_body' => t('Resource Body'),
                                    'resource_path' => t('附件路径'),
                                    'resource_type' => t('类型'),
                                    'created' => t('Created'),
                                    'updated' => t('Updated'),
                                    'creator' => t('Author'),
                ...
                protected function beforeSave()
                {
                        if (parent::beforeSave())
                        {
                                if ($this->isNewRecord)
                                {
                                        $this->created = $this->updated = SYS_TIME;
                                        $this->creator = user()->id;
                                }
                                else
                                {
                                        $this->updated = SYS_TIME;
                                }
                ...
                protected function afterDelete()
                {
                        ...
                        ObjectResource::model()->deleteAll('resource_id = :res', array(':res' => $this->resource_id));
                }
                ...
                public function getFullPath()
                {
                        ...
                        $object = new $class_storage;
                        
                        return $object->getFilePath($this->resource_path);
                }
